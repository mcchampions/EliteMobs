...
public class EnderDragonUnstuck implements Listener {
    ...
    public void onDragonEnterCombat(EliteMobEnterCombatEvent event) {
        ...
        if (event.getEliteMobEntity().getLivingEntity().getType() != EntityType.ENDER_DRAGON) return;
    ...
    public static class AggroPrevention implements Listener {
        ...
        public void onTarget(EntityTargetLivingEntityEvent event) {
            ...
            if (eliteEntity1 instanceof CustomBossEntity customBossEntity && customBossEntity.getCustomBossesConfigFields().isNeutral())
                return;
            if (event.getEntity().getType() == EntityType.WOLF && !((Wolf) event.getEntity()).isAngry() ||
                    event.getTarget() != null &&
                    event.getTarget().getType() == EntityType.WOLF && !((Wolf) event.getTarget()).isAngry())
                return;
